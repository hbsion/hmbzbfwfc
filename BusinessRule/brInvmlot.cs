...
{
    [Serializable]
    public class brInvmlot : IBrObject<inv_mlot>
    {
        ...
        void IBrObject<inv_mlot>.CheckAction(inv_mlot entity, ActionEnum action) { }

        private IQueryable<inv_mlot> GetByFilter(IQueryable<inv_mlot> q, Filter filter)
        {
            switch (filter.Field)
            {
        ...
        private IQueryable<inv_mlot> GetBySorter(IQueryable<inv_mlot> q, Sorter sorter)
        {
            switch (sorter.Field)
            {
                case FieldType.lot_no:
        ...
        public IEnumerable<inv_mlot> Query(Filter filter, Sorter sorter, int pageIndex, int pageSize)
        {
            return GetBySorter(GetByFilter(db.inv_mlot, filter), sorter).Skip(pageIndex * pageSize).Take(pageSize);
        }

        public int Count(Filter filter)
        {
            return GetByFilter(db.inv_mlot, filter).Count();
        }

        public inv_mlot Retrieve(string inCode)
        {
            return db.inv_mlot.FirstOrDefault(x => x.lot_no == inCode);
        }

        public inv_mlot Retrieve(string inCode, string inunitcode)
        {
            return db.inv_mlot.FirstOrDefault(x => x.lot_no == inCode && x.unitcode == inunitcode);
        }


        public inv_mlot Retrieve(int inCode)
        {
            return db.inv_mlot.FirstOrDefault(x => x.fid == inCode);
        }

        public inv_mlot Rallpart(string inunitcode)
        {
            return db.inv_mlot.FirstOrDefault(x => x.unitcode == inunitcode);
        }

        private IQueryable<inv_mlot> GetAll(QueryContext context)
        {
            IQueryable<inv_mlot> ret = db.inv_mlot;
        ...
        public IEnumerable<inv_mlot> Query(string inunitcode)  //返回xxx产品
        {
        ...
        public IEnumerable<inv_mlot> Query(string inlot_no, string inunitcode)  //返回xxx产品
        {
        ...
        public void Insert(inv_mlot entity)
        {
            lock (db)
            {
                ...
                try
                {
                    ...
                    if (db.inv_mlot.FirstOrDefault(x => x.fid == entity.fid) == null)
                    {
                        db.inv_mlot.InsertOnSubmit(entity);
                        db.SubmitChanges();
                    }
        ...
        public void Update(inv_mlot entity)
        {
            lock (db)
            {
                bool closeConn = false;
                ...
                {
                    db.Transaction = db.Connection.BeginTransaction();
                    inv_mlot dbdata = db.inv_mlot.FirstOrDefault(x => x.fid == entity.fid);
                    if (dbdata != null)
                        EntityUtilities.CloneEntity<inv_mlot>(entity, dbdata);
                    else
        ...
        public void Delete(int inCode)
        {
            lock (db)
            {
                ...
                {
                    db.Transaction = db.Connection.BeginTransaction();
                    inv_mlot dbdata = db.inv_mlot.FirstOrDefault(x => x.fid == inCode);
                    if (dbdata != null)
                        db.inv_mlot.DeleteOnSubmit(dbdata);
                    else
        ...
        public void Delete(inv_mlot entity)
        {
            this.Delete(entity.fid);
        }

        public inv_mlot NewEntity()
        {
            return new inv_mlot();
        }
    }
