<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Acusalelist.aspx.cs" Inherits="UI.Acusalelist" %>
...
id=A1 href="" 
target=_blank>终端销售明细查询</A></SPAN> </DIV>
...
<UL id=htmlDiv>
                            <LI class=music_ico>
 <DIV class=org style="font-size:12pt">条码编号：<%#Eval("bsnno")%> 客户：<%#Eval("cu_name")%> </DIV>
  <DIV class=org style="font-size:12pt">电话：<%#Eval("chetel")%> 车型：<%#Eval("chetype")%></DIV>
   <DIV class=org style="font-size:12pt">产品：<%#Eval("p_no")%> /<%#Eval("pname")%> </DIV> 
  <DIV class=org style="font-size:12pt">日期：<%#Convert.ToDateTime(Eval("ship_date")).ToString("yy-MM-dd")%> 数量：<%#Eval("mqty")%> </DIV>
