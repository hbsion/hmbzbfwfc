<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wx_cureScan.aspx.cs" Inherits="UI.wxws.wx_cureScan" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function addData(xtcu_no) {
            ...
            var index = layer.msg('正在扫码退货入库中...', { icon: 16 }); //0代表加载的风格，支持0-2
            if (hoid == 1) return;
            hoid = 1;
            ...
            $.ajax({
                ...
                cache: false,  
                async: false,
                data: { "cu_no": encodeURIComponent(xtcu_no), "optype": "cureScan", "snno": barcode, "ship_no": shipNo, "delyn": delyn },
                dataType: "json",
                timeout: 30000,
                success: function (data) {
                    hoid = 0;
                    if (data.success) {
                        $("#ret_prod").html(barcode + "退货成功");
                        $("#ret_mqty").html(parseInt($("#ret_mqty").html()) +1); //累加数量
    
                        layer.msg("条码:" + barcode + "退货成功");
