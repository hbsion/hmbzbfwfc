...
function initSelect() {
    ...
    var tmpl = ' <select name="provinceSelect" id="provinceSelect"  prompt="请选择省份" onchange="pselechang()"  style="width: 10em ;height: 2em;"  > ';
    ...
    for (var i = 0; i < selectData.citylist.length; i++) {
        ...
        tmpl = tmpl + '  <option value="' + city.p + '"> ' + city.p + ' </option>  ';
        // provinceSelect.add($C('option', {text:city.p, value:city.p}));
    }

    tmpl += ' </select>';


    $$('divprov').innerHTML = tmpl;


    var distList = getDistList(document.getElementById('provinceSelect').value);
    if (distList != null) {

        var tmpl2 = ' <select name="citySelect" id="citySelect"  prompt="请选择城市"  onchange="selectHidd()"  style="width: 10em ;height: 2em;"  > ';

        for (var j = 0; j < distList.length; j++) {
            var dist = distList[j];
            tmpl2 = tmpl2 + '  <option value="' + dist.n + '"> ' + dist.n + ' </option>  ';

        }
        tmpl2 += ' </select>';

        $$('divcity').innerHTML = tmpl2;
...
function selectHidd()
{
    document.getElementById('txtprov').value = document.getElementById('provinceSelect').value;
    document.getElementById('txtcity').value = document.getElementById('citySelect').value;
}
