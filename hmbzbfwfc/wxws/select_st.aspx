<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="select_st.aspx.cs" Inherits="UI.wxws.select_st" %>
    ...
    <script>
        ...
        $(function () {//加载完成后触发
            ...
            $("#listview").delegate('li', 'click', function () {
                ...
                if (localStorage["sttype"]=="2")
                {
                    parent.$("#nstInfo").val(no);
                    parent.$("#txtnst_no").val(name);
                }
                else
                {
                    parent.$("#stInfo").val(no);
                    parent.$("#txtst_no").val(name);
                }
        ...
        function loadCus(pageIndex) {
            ...
            $.ajax({
                ...
                type: "post",
                data: { "key": "st", "query": queryText, "PageIndex": pageIndex, "PageSize":15,"cu_no":username},
                dataType: "json",
                success: function (data) {
                    ...
                    if (json.success) {
                        ...
                        $.each(json.cont, function (i, v) {
                            ...
                            tmpHtml += "<label class='labcuname'>库别代号:<span>" + v.st_no + "</span></label>";
                            tmpHtml += "<label class='labPhone'>库别名称:<span>" + v.st_name + "</span></label>";
                            tmpHtml += "</a></li>";
                        });
