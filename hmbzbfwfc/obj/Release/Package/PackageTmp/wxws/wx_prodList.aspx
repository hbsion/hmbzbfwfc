<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wx_prodList.aspx.cs" Inherits="UI.wxws.wx_prodList" %>
...
<head id="Head1" runat="server">
    ...
    <script>
        ...
        $(function () {//加载完成后触发
            ...
            //点击打开产品详情,新打开一个页面,弹出即屏
            // $("#listview").delegate('li','click',function(){
            //var fid=$(this).attr("fid");
            // window.open("wx_prodDetails.aspx?fid="+fid+"");
            // });
        })
        //加载客户信息
        function loadData(pageIndex) {
            ...
            var username = getCookieVal(userinfo, "userName");
            $.ajax({
                url: "../App/appListOf.ashx",
                type: "post",
                data: { "key": "ship2", "query": queryText, "PageIndex": pageIndex, "PageSize": 10, "cu_no": username },
                dataType: "json",
                success: function (data) {
                    ...
                    if (json.success) {
                        ...
                        $.each(json.cont, function (i, v) {
                            tmpHtml += "<li data-icon='arrow-r' fid='" + v.fid + "'><a href='javascript:void(0)'>";
                            tmpHtml += "<label>条码:" + v.bsnno + "&nbsp;&nbsp;数量:" + v.mqty + "&nbsp;&nbsp;日期:" + v.ship_date + "</label>";
                            tmpHtml += "<label class='labcuno'><span>客户:" + v.cu_no + "</span>&nbsp;&nbsp;<span>姓名:" + v.cu_name + "</span></label>";
                            tmpHtml += "<label class='labcuno'><span>产品编号:" + v.p_no + "</span>&nbsp;&nbsp;<span>名称:" + v.pname + "</span></label>";
                            tmpHtml += "</a></li>";
                        });
