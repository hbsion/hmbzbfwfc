<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wx_fgiScan.aspx.cs" Inherits="UI.wxws.wx_fgiScan" %>
...
<head>
    ...
    <script src="js/cufgi.js" type="text/javascript"></script>
    ...
    <script type="text/javascript">
        ...
        function layerOpenWin(type) {
            if (type == "1") {
                ...
                onSelect("选择供应商", "select_vender.aspx", '100%', '85%');
            }
            if (type == "4") {
        ...
        function addData(xtcu_no) {
            ...
            var prodNo = $("#prodInfo").val();
            var barcode = $("#barcode").val();
            var lot_no = $("#txtlot_no").val();
            ...
            //判断元素是否为空
            if (prodNo.length == 0 && xtcu_no.length == 0) {
                layer.tips("请选择产品", $("#prodName"), { tips: 1, icon: 5 });
                return;
            }

            if (cusNo.length == 0) {
                layer.tips("请选择供应商", $("#cusInfo"), { tips: 1, icon: 5 });
                return;
            }
            var index = layer.msg('正在扫码入库中...', { icon: 16 }); //0代表加载的风格，支持0-2
            if (hoid == 1) return;
            hoid = 1;
            ...
            $.ajax({
                ...
                async: false,
                data: {
                    "cu_no": user_id, "optype": "fgiScan", "snno": barcode, "ship_no": shipNo,
                    "p_no": prodNo, "lot_no": lot_no, "st_no": st_no, "delyn": delyn, "shipcu_no": encodeURIComponent(cusNo)
                },
                dataType: "json",
                timeout: 30000,
                success: function (data) {
                    hoid = 0;
                    if (data.success) {
                        $("#ret_prod").html(barcode + "入库成功");
                        $("#ret_mqty").html(parseInt($("#ret_mqty").html()) + 1); //累加数量
    
                        layer.msg("条码:" + barcode + "入库成功");
        ...
        <div data-role="page" id="menu" style="position: relative; background-color: #ffffff">
            ...
            <div data-role="content">
                ...
                <div class="ub">
                    ...
                    <input type="text" style="margin-bottom: 0" placeholder="" runat="server" name="txtlot_no" id="txtlot_no" />
                
                </div>



                <div class="ub notOrdership"  id="prodDiv">
                    <label for="cusInfo">
                        产品:</label>
                    <input type="text" style="margin-bottom: 0" placeholder="*请选择产品" runat="server" id="prodInfo" />
                    <input value="选择产品" onclick="layerOpenWin('1')" type="button" id="select_prod" />
                ...
                <div class="ub notOrdership">
                    <label for="cusInfo">
                        供应商:</label>
                    <input type="text" style="margin-bottom: 0" placeholder="*请选择供应商" runat="server" name="cusInfo" id="cusInfo" />
                    <input value="选择供应商" onclick="layerOpenWin('2')" type="button" id="select_cus" />
                ...
                <div class="ub notOrdership">
                    <label for="cusInfo">
                        库别:</label>
                    <input type="text" style="margin-bottom: 0" placeholder="*请选择库别" runat="server" name="stInfo" id="stInfo" />
                    <input value="选择库别" onclick="layerOpenWin('4')" type="button" id="select_st" />
